...
| 🚀 **Session pre-warm + reaper** | Known group/private sessions spawn warm at startup (background — no CLI boot lag on the first message; list from `UB_PREWARM_CHATS` + auto-tracked `seen_chats`); idle sessions are killed after `UB_SESSION_IDLE_KILL` (default 1800s) to free RAM |
