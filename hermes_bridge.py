...
BRIDGE_PROMPT = (
    ...
    "ini). MAKSIMAL 2 "
    "KALI (2 kata kunci beda). Kalau hasil KOSONG → bilang jujur 'nggak nemu "
    "di history chat' + berhenti — JANGAN cari terus-terusan. HASIL SEARCH "
...
async def hermes_ask(chat_id: int, sender_name: str, chat_title: str, text: str, timeout: Optional[float] = None, on_update=None, on_tool=None, user_id: int = 0, msg_date=None, on_start=None, on_clarify=None, on_heartbeat=None, on_state=None, history_provider=None) -> str:
    ...
    async with lock:
        ...
        # ── HISTORY/ OBSERVED CONTEXT: inject tiap pesan — provider manage
        # sendiri: (1) session start → backfill API 50 pesan (snapshot),
        # (2) setelahnya → delta history.md (obrolan grup yang ke-lewat
        # sejak interaksi terakhir, marked "observed - context only").
        if history_provider is not None and config.GROUP_BACKFILL > 0:
            try:
                hist = await history_provider(chat_id)
                if hist:
                    prompt = (hist + "\n\n" + prompt)
            except Exception:
                pass
        ...
        for attempt in range(max_attempts):
            ...
            _ready = await _wait_for_prompt(name, timeout=15)
            if not _ready:
                _ready = await _wait_for_prompt(name, timeout=90)
            if not _ready:
                # session setengah mati — spawn ulang + coba lagi
                log.warning("CLI %s nggak siap — respawn session", name)
                try:
                    await _run("kill-session", "-t", name)
                except Exception:
                    pass
                ...
                try:
                    ...
                    _ready = False
            if not _ready:
                raise AgentError("CLI session nggak bisa siap setelah respawn")
            ...
            try:
                for _ in range(15):
                    ...
                    if marker and marker[:40] in _p.replace("\n", " "):
                        _delivered = True
                        break
                    if "❯" not in _p.replace("\n", " ") and ("synthesizing" in _p or "mulling" in _p):
                        _delivered = True  # CLI udah proses (status ganti) — aman
                        break
            except Exception:
                _delivered = True  # jangan blokir kalau capture error
            if not _delivered:
                raise AgentError("prompt nggak ke-deliver ke CLI (marker nggak muncul)")
            try:
