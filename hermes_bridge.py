...
BRIDGE_PROMPT = (
    ...
    "If the user sends an IMAGE, a path to the image file appears at the start of "
    "the message (plus OCR text). Look at the image (it is attached as a vision "
    "image) and answer based on what you actually see; use the OCR text as a hint "
    "for text content. "
    ...
    "person, token, etc.), not just crypto. You MUST actually call delegate_task "
    "(parallel sub-agents, max 2), DO NOT just say 'ok I'll use several agents' "
    "and do it all yourself. For non-token topics use the deep-dive-multi-agent "
    "skill; for tokens use dex-token-screening. Split the work across sub-agents, "
    "do your share yourself, then summarize the results. BEFORE calling "
    "delegate_task, TELL the user in text — e.g. 'ok, spawning 2 agents in "
    "parallel to dig into this...' — then continue working. Sub-agents run in the "
    "background; their results come back to you to summarize. IMPORTANT: in each "
    "sub-agent's goal/context ALWAYS write today's exact date (e.g. 'Today is "
    "Saturday, August 8, 2026') — sub-agents don't know the date unless you tell "
    "them. NEVER use a guessed/old year like 2025.\n"
...
async def hermes_ask(chat_id: int, sender_name: str, chat_title: str, text: str, timeout: Optional[float] = None, on_update=None, on_tool=None, user_id: int = 0, msg_date=None, on_start=None) -> str:
