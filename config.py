...
SESSION_FILE = os.environ.get("SESSION_FILE", "my_userbot")
...
HERMES_TOOLSETS = os.environ.get(
    "UB_HERMES_TOOLSETS", "skills,memory,web,todo,session_search,terminal,file,browser,cronjob,delegation"
)
HERMES_TIMEOUT = float(os.environ.get("UB_HERMES_TIMEOUT", "120"))  # detik IDLE (nggak ada aktivitas) sebelum dianggap macet
HERMES_HARD_TIMEOUT = float(os.environ.get("UB_HERMES_HARD_TIMEOUT", "600"))  # batas absolut maksimal nunggu agent
...
HIDE_TOOL_BUBBLES = set(
    os.environ.get("UB_HIDE_TOOLS", "session_search,memory,todo").split(",")
)  # tool internal yang nggak usah dimunculin (spam)
...
ELEVENLABS_VOICE_ID = os.environ.get("ELEVENLABS_VOICE_ID", "your_elevenlabs_apikey")
...
)  # balasan kalau ada yang nyuruh hal berbahaya ke server (bisa diubah via env)
