...
async def _ocr_event_image(event) -> str:
    ...
    try:
        ...
        if not path or not os.path.exists(path):
            ...
            return ""
        # resize kalau kegedean (tesseract lebih cepet & akurat di ukuran wajar)
        try:
...
async def on_incoming(event):
    ...
    # ── Media gambar → OCR, masuk ke konteks agent (kayak Hermes asli lihat gambar)
    if _is_image_media(event):
        ocr = await _ocr_event_image(event)
        if ocr:
            text = (text + f"\n\n[Gambar dari user — hasil OCR:\n{ocr}]").strip()
        else:
            text = (text + "\n\n[Gambar dari user — OCR gagal/nggak ada teks]").strip()
