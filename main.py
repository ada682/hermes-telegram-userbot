...
async def _keep_typing(chat_id: int):
    """Renew Telegram typing indicator tiap 2.5 detik (Telegram expire ~5s —
    2.5s + latency API (0.5-2s) = 4.5s < 5s → typing nggak sempet expire,
    nggak kedip. Typing per-chat: SEMUA chat yang lagi diproses nampilin
    typing-nya sendiri (Telegram handle per-chat — bukan satu akun satu
    typing)."""
...
async def _run_agent(event):
    ...
    # ── FIX TYPING LAMA (17:22): typing harus MUNCUL INSTAN pas user chat!
                ...
                async def _on_state(state_key: str, detail: str = None):
                    ...
                    try:
                        ...
                        if _txt:
                            ...
                            # thinking/prewarm bubble: DIHAPUS pas reply bubble
                            # masuk (bukan auto-delete 12s — indikator ngikutin run)
                            if state_key in ("thinking", "prewarm"):
                                thinking_msg = _m
                                # ── SAFETY NET (17:52): kalau deletion pas reply
                                # GAGAL (error silent!) → PRE WARM/THINKING self-
                                # delete 10 menit (nggak nempel selamanya!)
                                asyncio.create_task(_delete_later(600, _m))
                                return
                    ...
                    try:
                        if hb_msg is None:
                            ...
                            try:
                                ...
                                # "Content was not modified" = teks SAMA — bubble
                                # lama udah bener, JANGAN bikin bubble baru (dulu
                                # kebanyakan → spam heartbeats).
